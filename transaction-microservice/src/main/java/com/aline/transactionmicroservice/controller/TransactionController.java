...
public class TransactionController {
    ...
    @Operation(description = "Start a transaction")
    @PostMapping
    public ResponseEntity<TransactionResponse> createTransaction(@Valid @RequestBody CreateTransaction createTransaction) {
        ...
        return ResponseEntity
                .created(location)
                .body(service.mapToResponse(transaction));
    }

    @Operation(description = "Process transaction using a ProcessTransaction DTO")
    @PutMapping(value = "/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseStatus(HttpStatus.OK)
    public Receipt processTransaction(@PathVariable long id) {
        Transaction transaction = service.getTransactionById(id);
